...
import static org.eclipse.rap.selenium.xpath.XPath.byName;
...
public class RapBot
{
    ...
    public void loadApplicationWithLogin(String url, String user, String pass)
    {
        loadApplicationWithLogin(url, byName("username"), byName("password"), byName("submit"), user, pass);

    }

    public void loadApplicationWithLogin(String url, AbstractPath<?> userField, AbstractPath<?> passField, AbstractPath<?> submitField, String user, String pass)
    {
        selenium.open(url);
        selenium.waitForPageToLoad("10000");

        
        input(userField, user);
        input(passField, pass);
        click(submitField);
     ...
     *
     * NOTE: Even if the element is rendered in the DOM, a click may not be
     * registered if it is not visible on screen.
     *
     * @param xpath
     */
    ...
    private void click(String xpath)
    {
        ...
        // }
        // } catch( Exception exception ) {
        // // This should never happen, but sometimes selenium does not find
        // elements but driver does??
        // // use JS instead?
     ...
     *
     * POSITIVE values scroll UP, i.e towards the first line. NEGATIVE values
     * scroll DOWN, i.e away from the first line.
     *
     * @param xpath
     * @param delta
     */
    ...
    /**
     * Emulates a key event in JavaScript. Has no effect on text fields or
     * focus.
     *
     * @param xpath
     * @param key
     *            a single character, or any value from {@link KeyIdentifier}
     */
    ...
    // can't be used to
    // control most RAP widgets. Also, selenium.focus does not always work on
    // RAP
    // widgets, possibly
    ...
    {
        checkElementCount(xpath);
        String script = "(function(){var el =  " + xpathToJs(xpath) + ";var widget = rwt.event.EventHandlerUtil.getOriginalTargetObject( el );" + "RapBot.TestUtil.press( widget,\"" + key + "\" );" + "}());";
    ...
    public void sendEnterKey(AbstractPath<?> xpath)
    {
        sendEnterKey(xpath.toString());
    }
    ...
    public void sendEnterKey(String xpath)
    {
        checkElementCount(xpath);
        WebElement inputElement = driver.findElement(By.xpath(xpath));
    ...
    {
        // There may be a short delay between the user input and the widget
        // sending
        // the request.
        // In most cases the request is hard-coded to 60ms, though the JS timer
        // isn't very precise.
        // In a few cases like Modify event the delay is much longer and this
        // method
        // won't suffice.
        try
        {
    ...
    /**
     * Returns true while the RAP client is waiting for the server to process an
     * event.
     */
    ...
    public boolean isElementAvailable(String xpath)
    {
        ...
        // // NOTE [tb] : I do not know why, but an element can be "not present"
        // and
        // // have an count == 1, or be "present" but not found by isVisible...
        // try {
        // return count > 0 && selenium.isElementPresent( xpath ) &&
    ...
    public boolean isElementDisplayed(String xpath)
    {
        ...
        try
        {
            return driver.findElements(By.xpath(xpath)).size() > 0 && driver.findElements(By.xpath(xpath)).get(0).isDisplayed();
        }
        catch (NoSuchElementException e)
        {
            return false;
    ...
    public boolean isElementDisplayed(AbstractPath<?> xpath)
    {
        return isElementDisplayed(xpath.toString());
    }
    ...
    public String getValue(AbstractPath<?> xpath)
    {
        return getValue(xpath.toString());
    }
